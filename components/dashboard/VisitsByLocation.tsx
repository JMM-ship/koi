    ...
    if (data && userInfo) {
      const processPlanDetails = () => {
        try {
          ...
          const featuresList = [];
          if (creditCap > 0) {
            featuresList.push({ name: (_t('packages.features.creditCap', { count: creditCap.toLocaleString() }) || `Credit Cap: ${creditCap.toLocaleString()}`), included: true });
          }
          if (recoveryRate > 0) {
            featuresList.push({ name: (_t('packages.features.recoveryPerHour', { count: recoveryRate.toLocaleString() }) || `Recovery Rate: ${recoveryRate.toLocaleString()}/hour`), included: true });
          }
          if (dailyUsageLimit > 0) {
            featuresList.push({ name: (_t('packages.features.dailyMaxUsage', { count: dailyUsageLimit.toLocaleString() }) || `Daily Usage Limit: ${dailyUsageLimit.toLocaleString()}`), included: true });
          }
          if (manualResetPerDay > 0) {
            featuresList.push({ name: (_t('packages.features.manualResetPerDay', { count: String(manualResetPerDay) }) || `Manual Resets: ${manualResetPerDay}/day`), included: true });
          }
