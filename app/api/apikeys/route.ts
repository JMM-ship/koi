...
function hashApiKey(rawKey: string): string {
  ...
  return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
      ...
      // 5) 一致性校验：若不一致，优先自愈（仅限池记录，避免影响已分配记录）
      const computedHash = hashApiKey(actualKey)
      const computedPrefix = actualKey.substring(0, 7)
      if (
        (availableKey.keyHash && availableKey.keyHash !== computedHash) ||
        (availableKey.prefix && !actualKey.startsWith(availableKey.prefix))
      ) {
        // 并发安全：只更新池记录（ownerUserId 为空且仍为 active）
        try {
          const fix = await prisma.apiKey.updateMany({
            where: { id: availableKey.id, ownerUserId: null, status: 'active' },
            data: { keyHash: computedHash, prefix: computedPrefix },
          })
          if (fix.count === 0) {
            return NextResponse.json(
